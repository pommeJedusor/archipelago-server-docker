...
class Permission(ByValue, enum.IntFlag):
    ...
    auto_enabled = (
        0b111  # 7, forces use after goal completion, allows manual use any time
    )
...
def _scan_for_TypedTuples(obj: typing.Any) -> typing.Any:
    if isinstance(obj, tuple) and hasattr(
        obj, "_fields"
    ):  # NamedTuple is not actually a parent class
        data = obj._asdict()
...
def convert_to_base_types(obj: typing.Any) -> _base_types:
    if isinstance(obj, (tuple, list, set, frozenset)):
        ...
        return {
            convert_to_base_types(key): convert_to_base_types(value)
...
def get_any_version(data: dict) -> Version:
    ...
    }  # .NET version classes have capitalized keys
...
custom_hooks = {"Version": get_any_version}
...
class HandlerMeta(type):
    def __new__(mcs, name, bases, attrs):
        ...
        handlers.update(
            {
                handler_name[len(trigger) :]: method
                for handler_name, method in attrs.items()
                if handler_name.startswith(trigger)
            }
        )
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.handlers = {
                name: method.__get__(self, type(self))
...
class JSONtoTextParser(metaclass=HandlerMeta):
    ...
    def _handle_location_id(self, node: JSONMessagePart):
        ...
        node["text"] = self.ctx.location_names.lookup_in_slot(
            location_id, node["player"]
        )
...
color_codes = {
    ...
    "slateblue": 34,
    "salmon": 31,
}  # convert ui colors to terminal colors
...
def add_json_item(
    parts: list, item_id: int, player: int = 0, item_flags: int = 0, **kwargs
) -> None:
    parts.append(
        {
            "text": str(item_id),
...
def add_json_location(parts: list, location_id: int, player: int = 0, **kwargs) -> None:
    parts.append(
        {
            "text": str(location_id),
            "player": player,
            "type": JSONTypes.location_id,
            **kwargs,
        }
    )
...
def add_json_hint_status(
    parts: list, hint_status: HintStatus, text: typing.Optional[str] = None, **kwargs
):
    parts.append(
        {
            "text": (
                text if text != None else status_names.get(hint_status, "(unknown)")
            ),
            "hint_status": hint_status,
            "type": JSONTypes.hint_status,
            **kwargs,
        }
    )
...
class Hint(typing.NamedTuple):
    ...
    def as_network_message(self) -> dict:
        ...
        return {
            ...
            "item": NetworkItem(
                self.item, self.location, self.finding_player, self.item_flags
            ),
            "found": self.found,
        }
...
class _LocationStore(
    dict, typing.MutableMapping[int, typing.Dict[int, typing.Tuple[int, int, int]]]
):
    def __init__(
        self,
        values: typing.MutableMapping[
            int, typing.Dict[int, typing.Tuple[int, int, int]]
        ],
    ):
    ...
    def find_item(
        self, slots: typing.Set[int], seeked_item_id: int
    ...
    def get_checked(
        ...
        if not checked:
            ...
            if slot not in self:
                raise KeyError(slot)
            return []
        return [location_id for location_id in self[slot] if location_id in checked]
    ...
    def get_missing(
        ...
        return [location_id for location_id in self[slot] if location_id not in checked]
    ...
    def get_remaining(
        ...
        return sorted(
            [
                (player_locations[location_id][1], player_locations[location_id][0])
                for location_id in player_locations
...
if (
...
):  # type-check with pure python implementation until we have a typing stub
    ...
    try:
        ...
        if os.path.isfile("_speedups.pyx") and os.path.getctime(
            _speedups.__file__
        ) < os.path.getctime("_speedups.pyx"):
            warnings.warn(
                f"{_speedups.__file__} outdated! "
        ...
        try:
            ...
            warnings.warn(
                "_speedups not available. Falling back to pure python LocationStore. "
