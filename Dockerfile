...
RUN wget https://github.com/ArchipelagoMW/Archipelago/releases/download/0.6.6/Archipelago_0.6.6_linux-x86_64.tar.gz &&\
  tar -xf Archipelago_0.6.6_linux-x86_64.tar.gz &&\
  rm Archipelago_0.6.6_linux-x86_64.tar.gz

RUN echo "if [ ! -f /archipelago/Archipelago/output/AP_*.zip ]; then" >> /archipelago/startup.sh &&\
    echo "  echo 'test2'" >> /archipelago/startup.sh &&\
